...
class MiMotionRunner {
  ...
  async loginAndPostStep(step) {
    ...
    const [loginToken, userId] = await this.login();
    if (loginToken === 0) {
      return ["登陆失败！", false];
    }
    ...
    // 用正则替换 JSON 字符串里的 date 和 ttl（也就是步数部分）
    const findDate = /.*?date%22%3A%22(.*?)%22%2C%22data.*?/;
    const findStep = /.*?ttl%5C%22%3A(.*?)%2C%5C%22dis.*?/;

    // 从原始 dataJson 中 extract 出来的旧值，分别替换为 today 和 step
    const matchDate = findDate.exec(dataJson);
    if (matchDate && matchDate[1]) {
      dataJson = dataJson.replace(matchDate[1], today);
    }
    const matchStep = findStep.exec(dataJson);
    if (matchStep && matchStep[1]) {
      dataJson = dataJson.replace(matchStep[1], step);
    }
    ...
    const body = `userid=${encodeURIComponent(userId)}&last_sync_data_time=1597306380&device_type=0&last_deviceid=DA932FFFFE8816E7&data_json=${dataJson}`;
    ...
    try {
      ...
      return [`请求异常: ${err.message}`, false];
    }

    const msg = responseJson.message || "无响应 message 字段";
    console.log('[ 当前时间： ] >', new Date().toLocaleString())
    return [`${this.user}--修改步数--${step}--[${msg}]`, true];
  }
}
